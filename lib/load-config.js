...
loadConfig.normalize = normalize

function normalize(config) {
	config.patterns = typeof config.patterns == 'string' ? config.patterns.split(',') : config.patterns || [];
	config.ignorePatterns = typeof config.ignorePatterns == 'string' ? config.ignorePatterns.split(',') : config.ignorePatterns || [];
	config.directories = typeof config.directories == 'string' ? config.directories.split(',') : config.directories || [];
	config.ignoreDirectories = typeof config.ignoreDirectories == 'string' ? config.ignoreDirectories.split(',') : config.ignoreDirectories || [];
